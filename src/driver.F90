...
!FAB           dens_aer, iaer_bc, iaer_pom, iaer_so4, iaer_soa, iaer_ncl, &
!           iaer_mom, iaer_dst
...
!                            (tmpfmom / dens_aer(iaer_mom))   )
                tmpdens = 1770. ! FAB revoir cette initialisation de toute facon
...
! call wateruptake
      call modal_aero_wateruptake_dr( state, pbuf,deltat ,istep )
...
!
      ...
      do i = 1, ntot_amode
       ...
       if(lptr_so4_a_amode(i)>0)  tmp_so4_aer(nstep,i) = q(1,1,lptr_so4_a_amode(i))
       if(lptr_nh4_a_amode(i)>0)  tmp_nh4_aer(nstep,i) = q(1,1,lptr_nh4_a_amode(i))
       if(lptr_no3_a_amode(i)>0)  tmp_no3_aer(nstep,i) = q(1,1,lptr_no3_a_amode(i))
       if(lptr_soa_a_amode(i)>0)  tmp_soa_aer(nstep,i) = q(1,1,lptr_soa_a_amode(i))
       if(lptr_pom_a_amode(i)>0)  tmp_pom_aer(nstep,i) = q(1,1,lptr_pom_a_amode(i))
       if(lptr_bc_a_amode(i)>0)   tmp_bc_aer(nstep,i)  = q(1,1,lptr_bc_a_amode(i))
       if(lptr_dust_a_amode(i)>0) tmp_dust_aer(nstep,i) = q(1,1,lptr_dust_a_amode(i))
       if(lptr_co3_a_amode(i)>0)  tmp_co3_aer(nstep,i) = q(1,1,lptr_co3_a_amode(i))
       if(lptr_ca_a_amode(i)>0)   tmp_ca_aer(nstep,i) = q(1,1,lptr_ca_a_amode(i))
       if(lptr_nacl_a_amode(i)>0) tmp_nacl_aer(nstep,i) = q(1,1,lptr_nacl_a_amode(i))
      end do
